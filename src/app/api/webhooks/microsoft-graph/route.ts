...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify the webhook is from Microsoft (you should implement proper verification)
    ...
    const webhookPayload: WebhookPayload = {
      ...
      eventType: body.value?.[0]?.changeType || 'file.updated',
      resourceId: body.value?.[0]?.resourceId || body.value?.[0]?.resource || '',
...
export async function GET(request: NextRequest) {
  // Webhook verification endpoint for Microsoft
